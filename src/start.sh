...
if [[ ! -z "$START_WEBUI" ]]; then
    ...
    echo "Starting WebUI"
    python /stable-diffusion-webui/webui.py --skip-python-version-check --skip-torch-cuda-test --skip-install --ckpt /model.safetensors --lowram --opt-sdp-attention --disable-safe-unpickle --listen --port 3000 --api --cors-allow-origins "*" --skip-version-check  --no-hashing --no-download-sd-model &
